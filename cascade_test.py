...
def load_data():
    ...
    y_train = np.load('../casia_surf_data/data_train_label.npy')

    images_train_rgb = images_train_rgb / 255
    images_train_dep = images_train_dep / 255
    images_train_ir = images_train_ir / 255
    images_test_rgb = np.load('../casia_surf_data/data_test_rgb.npy')
    images_test_dep = np.load('../casia_surf_data/data_test_dep.npy')
    images_test_ir = np.load('../casia_surf_data/data_test_ir.npy')
    y_test = np.load('../casia_surf_data/data_test_label.npy')
...
print(images_test[0].shape[0])
print(images_test[1].shape)
print(images_test[2].shape)
pcanet_ir = load_model(join("pretrained_model", "pcanet_ir_cascade.pkl"))
pcanet_dep = load_model(join("pretrained_model", "pcanet_dep_cascade.pkl"))
classifier_ir = load_model(join("pretrained_model", "classifier_ir_cascade.pkl"))
classifier_dep = load_model(join("pretrained_model", "classifier_dep_cascade.pkl"))
pcanet_rgb = load_model(join("pretrained_model", "pcanet_rgb_cascade.pkl"))
classifier_rgb = load_model(join("pretrained_model", "classifier_rgb_cascade.pkl"))
print(images_test[1].shape)
# dep = pcanet_dep.transform(images_test[1])
# y_pred = classifier_dep.predict(dep)
pred = []
for i in range(images_test[1].shape[0]):
    test_dep = images_test[1][i]
    test_ir = images_test[2][i]
    ...
    if float(y_pred_dep_p[0][0]) >= 0.65:
        ...
        if float(y_pred_dep_p[0][1]) >= 0.90:
            ...
            y_pred =(y_pred_dep_p[0][1]+y_pred_ir_p[0][1]+y_pred_rgb_p[0][1])/3
            if y_pred >= 0.5:
                print("Pred: Real, Actual:" + actual)
                y_pred = 1
                pred.append(y_pred)
